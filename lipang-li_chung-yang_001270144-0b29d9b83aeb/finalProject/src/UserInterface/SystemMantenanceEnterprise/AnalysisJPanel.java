...
public class AnalysisJPanel extends javax.swing.JPanel {
    ...
    public void populateAnalysisComboBox() {   
        ...
        analysisComboBox.addItem("Total Investment");
        analysisComboBox.addItem("Total Income");
        ...
        if(s.equalsIgnoreCase("Total Amount(Buy + Sell)")){
            ...
            for(Enterprise enterprise: ecoSystem.getEnterpriseNetwork().getEnterpriseList()){
                if(enterprise instanceof InvestmentEnterprise){
                    InvestmentOrg investmentOrg = (InvestmentOrg)enterprise.getOrganizationDirectory().searchOrgByType(Organization.Type.Investment);
                    for(WorkRequest workRequest: investmentOrg.getWorkQueue().getWorkRequestList()){
                        InvestmentWorkRequest iw = (InvestmentWorkRequest)workRequest;
                        totalAmount += iw.getInitialAmount();
                    }
                    dataset.setValue(totalAmount, "Amount", enterprise.getName());
                    totalAmount = 0;
                }
            }
            dataset.setValue(40, "Amount", "Goldman Sachs");
            dataset.setValue(30, "Amount", "Vanguard");
        }
        if(s.equalsIgnoreCase("Total Investment")){
            for(Enterprise enterprise: ecoSystem.getEnterpriseNetwork().getEnterpriseList()){
                if(enterprise instanceof InvestmentEnterprise){
                    InvestmentEnterprise ie = (InvestmentEnterprise)enterprise;               
                    dataset.setValue(ie.getTotalSpent().getDealPrice(), "USD", enterprise.getName());
                }
            }
            dataset.setValue(120000, "USD", "Goldman Sachs");
            dataset.setValue(35000, "USD", "Vanguard");
        }
        if(s.equalsIgnoreCase("Total Income")){
            for(Enterprise enterprise: ecoSystem.getEnterpriseNetwork().getEnterpriseList()){
                if(enterprise instanceof InvestmentEnterprise){
                    InvestmentEnterprise ie = (InvestmentEnterprise)enterprise;               
                    dataset.setValue(ie.getTotalEarned().getDealPrice(), "USD", enterprise.getName());
                }
            }
            dataset.setValue(190000, "USD", "Goldman Sachs");
            dataset.setValue(50000, "USD", "Vanguard");
        }
        String a = "USD$";
